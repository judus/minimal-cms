...
class AssetsController
{
    ...
    protected function serve($fileSegmentPath)
    {
        ...
        $this->response->header('Etag: ' . $etagFile);

        $ifModifiedSince = !is_null(getenv('HTTP_IF_MODIFIED_SINCE')) ? getenv('HTTP_IF_MODIFIED_SINCE') : false;
        $etagHeader = !is_null(getenv('HTTP_IF_NONE_MATCH')) ? trim(getenv('HTTP_IF_NONE_MATCH')) : false;

        if (@strtotime(getenv('HTTP_IF_MODIFIED_SINCE')) == $lastModified || $etagHeader == $etagFile) {
